...
function agregarCurso(hString) {
    ...
    for(let m of matricula) {
        if(m.horario.dia === horario.dia) {
            const eInicio = parseInt(m.horario.hora_inicio.replace(':',''));
            const eFin = parseInt(m.horario.hora_fin.replace(':',''));
            if(nInicio < eFin && nFin > eInicio) return mostrarToast("⛔ Cruce de horario con " + m.curso.nombre, "bg-danger");
        }
    }
...
function renderizarTablaMisCursos() {
    ...
    matricula.forEach(m => {
        tbody.innerHTML += `<tr><td>${m.curso.codigo}</td><td>${m.curso.nombre}</td><td>${m.horario.seccion}</td><td>${m.horario.profesor}</td><td>${m.horario.dia} ${m.horario.hora_inicio}</td><td>${m.curso.creditos}</td></tr>`;
    });
}
...
function dibujarHorarioEnGrid() {
    ...
    for(let h=8; h<=16; h++) {
        grid.innerHTML += `<div class="time-header text-dark bg-white border-0">${h}:00</div>`;
        for(let d=1; d<=6; d++) {
            ...
            const clase = matricula.find(m => {
                ...
                return m.horario.dia === diaNombre && h >= ini && h < fin;
            });
            if(clase) {
                grid.innerHTML += `
                <div class="course-cell p-0">
                    <div class="event-block">
                        <div style="font-weight: bold; line-height: 1.1; margin-bottom: 2px;">${clase.curso.nombre}</div>
                        <div>${clase.horario.hora_inicio.slice(0,5)} - ${clase.horario.hora_fin.slice(0,5)}</div>
                    </div>
                </div>`;
            } else {
